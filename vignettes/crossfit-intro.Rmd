...
-   multiple methods in the same run,
...
-   `folds` and `repeats` define **K-fold cross-fitting repeated R
    times**,
-   `eval_fold` specifies the width, in folds, of the target's
    evaluation window,
...
-   `K`, `K_required`, `method`, `plan` – extra diagnostics.
...
``` r
res_pred <- crossfit(data2, m_ens)

# estimate is now a prediction function
f_hat <- res_pred$estimate
...
-   the target's evaluation window contains `eval_fold` folds,
...
Failure handling is configured for each method:

``` r
default_failure_control <- crossfit_failure_control()
```

By default, the first panel error fails the current repetition. To attempt all
panels and pass failed panel results as `NA_real_` to the panel aggregator, set
`fail_repetition_on_error = FALSE`:

``` r
available_mean <- function(xs) mean(unlist(xs), na.rm = TRUE)

m_available <- create_method(
  target        = target_mse,
  list_nuisance = list(nuis_y = nuis_y),
  folds         = 4,
  repeats       = 5,
  ...
  aggregate_repeats = available_mean,
...
Set `prune_on_shared_fit_failure = TRUE` to skip a method when a structurally
identical nuisance fit has already failed in the same repetition.
`max_failed_repetitions` sets how many failed repetitions are allowed before
...
-   Explore `results` (or `per_method` from `crossfit_multi()`) and `plan` if you
    need to:
